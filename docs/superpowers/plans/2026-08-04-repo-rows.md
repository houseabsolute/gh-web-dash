...
**Goal:** Replace the chronological run feed with one row per repository, showing the latest run of each workflow, expandable to per-workflow run history.

**Architecture:** No schema change — the same `runs` rows answer a different question. A window function picks the latest run per (repo, workflow); `store` aggregates those into repository summaries with a rolled-up health. The page becomes a two-file script: `app.js` for polling and state, `render.js` for drawing rows and expansions.
...
**Deviation from the spec, deliberate:** the spec names a
`workflow_history(repo, workflow, limit)` method. The endpoint is per-repository,
so a per-workflow method would mean N queries per request. This plan implements
`repo_history(repo, per_workflow)` — one query returning every workflow's history
for that repository. Task 7 amends the spec to match.

---

### Task 1: Move store tests out of `src/store.rs`

`src/store.rs` is 575 lines, roughly half of it tests, and this plan adds two
more queries. `Store`'s API is fully public and the tests use nothing private, so
...
Cut the entire `#[cfg(test)] mod tests { ... }` block from the end of
`src/store.rs` and paste its **contents** (not the `mod tests` wrapper, not the
`#[cfg(test)]` attribute) into a new `tests/store.rs`.
...
Run: `cargo test`
Expected: the same number of tests pass as before the move (56 total). The store
tests now report under `tests/store.rs` rather than the unit-test binary.

Run: `wc -l src/store.rs`
Expected: roughly 275 lines.
...
`Health` is the rolled-up state of a run, a workflow, or a repository. Its
ordering *is* the roll-up rule: a repository's health is the maximum over its
workflows, so `Ord` must run least-bad to worst.
...
Expected: FAIL — `Health` and `repo_summaries` not found.
...
One query returns every workflow's recent runs for a repository. A method per
workflow would mean N queries per request.
...
Run: `cargo test --test store`
Expected: FAIL — `repo_history` not found.
...
Expected: FAIL — `/api/repos` does not exist.
...
Note: a missing `repo` parameter makes axum's `Query<HistoryParams>` extractor
reject with 400 automatically, which is what
`history_requires_a_repo_parameter` asserts.

- [ ] **Step 4: Delete the feed queries**

From `src/store.rs`, delete: the `RunQuery` struct and its `Default` impl, the
`recent_runs` method, and the `workflow_names_for_query` method.

From `tests/store.rs`, delete every test that referenced them —
`recent_runs_are_newest_first`, `failures_only_excludes_success_and_in_progress`,
`limit_caps_results`, `workflow_names_are_distinct_and_sorted`, and any others
calling `recent_runs` or `workflow_names_for_query` — and drop `RunQuery` from
the import. Keep `upsert_is_idempotent_and_updates_in_place`, but rewrite its
assertions to use `repo_summaries` instead of `recent_runs`:
...
Run: `cargo test`
Expected: everything passes. The build will fail first on `src/assets/render.js`
...
td.caret { width: 1%; opacity: 0.45; font-size: 10px; padding-right: 0; }
td.status-cell { width: 1%; padding-right: 0; }
...
td.exp { padding: 2px 12px 10px 34px; white-space: normal; }
.wfblock { padding: 5px 0; }
.wfname { font-weight: 600; font-size: 12px; margin-right: 8px; }
...
a.sub { display: block; font-size: 12px; opacity: 0.75; padding-top: 3px; }
.loading { font-size: 12px; opacity: 0.6; padding: 4px 0; }
```
...
Run: `cargo test`
Expected: all pass — `github.rs`'s tests do not assert on the page size.
...
The expansion behaviors are JavaScript with no test harness. They must be
verified by driving the real page, not asserted from the code.
...
Wait for the first sync to populate rows (it can take several minutes on a cold
cache), then check:

1. One row per repository, sorted newest activity first.
2. A healthy repository reads "2 workflows"; a broken one names the broken
   workflow in a pill and counts the rest as "+N ok".
3. Clicking a row expands it; a second click collapses it.
4. The expansion shows one block per workflow, each with a history strip and a
   detail line.
5. **The strip's leftmost segment is the most recent run**, and its colour
   matches the repository's leading dot when that workflow is the worst one.
6. **Leave a row expanded for over 15 seconds.** It must stay open across at
   least one poll — this is the behavior most likely to be broken.
7. Expand a second row without collapsing the first.
8. Click a strip segment: it opens that run on github.com in a new tab **and the
   row stays open**.
9. Click "Failures only": only broken repositories remain. Click "All": everything
   returns.
10. Click "Refresh now": the header's synced-at time resets.

- [ ] **Step 3: Report honestly**

If any of the ten fail, fix it and re-verify rather than reporting success. Say
which ones you actually observed.
...
- **SQLite window functions** need 3.25+ (2018). The `bundled` feature compiles a
  current SQLite, so this is not a concern — but do not switch to a system
  SQLite without checking.
- **Do not reintroduce the feed.** `/api/runs`, `recent_runs`, and the workflow
  chips are deleted on purpose. If something seems to need them, that is a
  design question worth raising, not a reason to restore them quietly.
- **`Health`'s ordering is load-bearing.** `Ord` is what makes a repository's
  health the max over its workflows. Reordering the variants silently changes
  which colour a repository shows.
